import { Component, ComponentInterface, Element, Event, EventEmitter, Host, Method, Prop, h, Listen, forceUpdate, readTask } from '@stencil/core';
import { Color } from '../../interface';
import { hostContext, setClassList } from '../../utils/style';

import { ScrollBaseDetail, ScrollDetail } from './content.interface';
...
export class Content implements ComponentInterface {
  ...
  @Element() el!: HTMLBkkrContentElement;
  ...
  /**
   * Because of performance reasons, bkkrScroll events are disabled by default, in order to enable them
   ...
   */
  @Event() bkkrScrollStart!: EventEmitter<ScrollBaseDetail>;

  /**
   * Emitted while scrolling. This event is disabled by default.
   * Look at the property: `scrollEvents`
   */
  @Event() bkkrScroll!: EventEmitter<ScrollDetail>;

  /**
   * Emitted when the scroll has ended.
   */
  @Event() bkkrScrollEnd!: EventEmitter<ScrollBaseDetail>;
  ...
  private onScroll(ev: UIEvent) {
    ...
    if (!this.queued && this.scrollEvents) {
      ...
      readTask(ts => {
        ...
        this.bkkrScroll.emit(this.detail);
      });
    }
  }
    ...
    return (
      <Host
        class={setClassList(this.color, {
          'content-sizing': hostContext('bkkr-popover', this.el)
        })}
      >
        <main
          class={{
...
const getPageElement = (el: HTMLElement) => {
  const tabs = el.closest('bkkr-tabs');
  if (tabs) {
    return tabs;
  }
  const page = el.closest('bkkr-root,bkkr-page,.bkkr-page,page-inner');
  if (page) {
    return page;
  }
