 ...
 */
export const enterAnimation = (baseEl: HTMLElement, ev?: Event): Animation => {
    let originY = 'top';
    let originX = 'left';
    ...
    if (popoverCSS.left < POPOVER_BODY_PADDING + 25) {
      checkSafeAreaLeft = true;
      popoverCSS.left = POPOVER_BODY_PADDING;
    } else if (
      contentWidth + POPOVER_BODY_PADDING + popoverCSS.left + 25 > bodyWidth
    ) {
      // Ok, so we're on the right side of the screen,
      // but now we need to make sure we're still a bit further right
      // cus....notchurally... Again, 25 is random. It works tho
      checkSafeAreaRight = true;
      popoverCSS.left = bodyWidth - contentWidth - POPOVER_BODY_PADDING;
      originX = 'right';
    }
    ...
    if (targetTop + targetHeight + contentHeight > bodyHeight && targetTop - contentHeight > 0) {
      ...
      contentEl.style.bottom = POPOVER_BODY_PADDING + '%';
    }
    ...
    if (checkSafeAreaLeft) {
      contentEl.style.left = `calc(${popoverCSS.left}px + var(--safe-area-left, 0px))`;
    }
  
    if (checkSafeAreaRight) {
      contentEl.style.left = `calc(${popoverCSS.left}px - var(--safe-area-right, 0px))`;
    }
