...
const getNamedUrl = (iconName: string, iconType: 'regular' | 'solid' | 'brands' = 'solid') => {
    const url = getIconMap().get(iconName);
    if (url) {
        return url;
    }
    return getAssetPath(`fontawesome/${iconType}/${iconName}.svg`);
};
