import { Component, ComponentInterface, Element, Host, h, State, Prop, Method, Listen, EventEmitter, Event } from '@stencil/core';

import { MenuChangeEventDetail, MenuI } from './menu.interface';
import { Side } from '../../interface';
import { getTimeGivenProgression } from '../../utils/animation/cubic-bezier';
import { Animation } from '../../utils/animation/animation-interface';
import { Gesture, GestureDetail, createGesture, GESTURE_CONTROLLER } from '../../utils/gesture';
import { assert, clamp } from '../../utils/helpers';
...
export class Menu implements ComponentInterface, MenuI {
  ...
  @Element() el!: HTMLBkkrMenuElement;
   ...
   */
  @Prop({ mutable: true }) type?: "overlay" | "reveal" | "push";
   ...
   */
  @Event() bkkrWillOpen!: EventEmitter<void>;

  /**
   * Emitted when the menu is about to be closed.
   */
  @Event() bkkrWillClose!: EventEmitter<void>;
  /**
   * Emitted when the menu is open.
   */
  @Event() bkkrDidOpen!: EventEmitter<void>;

  /**
   * Emitted when the menu is closed.
   */
  @Event() bkkrDidClose!: EventEmitter<void>;

  /**
   * Emitted when the menu state is changed.
   * @internal
   */
  @Event() protected bkkrMenuChange!: EventEmitter<MenuChangeEventDetail>;
  ...
  async componentDidLoad() {
    this.bkkrMenuChange.emit({ disabled: this.disabled, open: this._isOpen });
    this.updateState();
  }
  ...
  private async startAnimation(shouldOpen: boolean, animated: boolean): Promise<void> {
    ...
    const easingReverse = 'cubic-bezier(1, 0, 0.68, 0.28)';
  ...
  private canStart(detail: GestureDetail): boolean {
    ...
    const isModalPresented = !!document.querySelector('bkkr-modal.show-modal');
  ...
  private beforeAnimation(shouldOpen: boolean) {
    ...
    if (shouldOpen) {
      this.bkkrWillOpen.emit();
    } else {
      this.bkkrWillClose.emit();
    }
  }
