import { Component, ComponentInterface, Host, h, Prop, EventEmitter, forceUpdate, Method, State, Watch, Event, Element } from '@stencil/core';

import { AutocompleteTypes, Color, StyleEventDetail } from '../../interface';
import { debounceEvent, raf } from '../../utils/helpers';
import { setClassList } from '../../utils/style';
import { SearchbarChangeEventDetail } from './searchbar.interface';
...
export class Searchbar implements ComponentInterface {
  ...
  /**
   * Set the amount of time, in milliseconds, to wait to trigger the `bkkrChange` event after each keystroke. This also impacts form bindings such as `ngModel` or `v-model`.
   */
  @Prop() debounce = 250;

  @Watch('debounce')
  protected debounceChanged() {
    this.bkkrChange = debounceEvent(this.bkkrChange, this.debounce);
  }
   ...
   */
  @Event() bkkrInput!: EventEmitter<KeyboardEvent>;

  /**
   * Emitted when the value has changed.
   */
  @Event() bkkrChange!: EventEmitter<SearchbarChangeEventDetail>;
   ...
   */
  @Event() bkkrClear!: EventEmitter<void>;
  ...
  /**
   * Sets focus on the specified `bkkr-searchbar`. Use this method instead of the global
   * `input.focus()`.
   */
  @Method()
  async setFocus() {
  ...
  private onInput = (ev: Event) => {
    ...
    this.bkkrInput.emit(ev as KeyboardEvent);
  }
  ...
  render() {
    ...
    const cancelButton = (this.showCancelButton !== 'never') && (
      <button
        ...
        type="button"
        tabIndex={!shouldShowCancelButton ? -1 : undefined}
    ...
    return (
      ...
      >
        ...
        <div class="searchbar-input-container">
          ...
          <bkkr-icon aria-hidden="true" icon='search' lazy={false} class="searchbar-search-icon"></bkkr-icon>
          ...
          >
            <bkkr-icon aria-hidden="true" icon='times-circle' lazy={false} class="searchbar-clear-icon"></bkkr-icon>
          </button>
        </div>
        {cancelButton}
      </Host>
