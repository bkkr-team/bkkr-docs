...
export class Hint implements ComponentInterface {
  ...
  /**
   * The position determines where and how the hint behaves inside an item.
   */
  ...
  private emitColor() {
    ...
    this.bkkrColor.emit({
      'item-hint-color': color !== undefined,
  ...
  private emitStyle() {
    ...
    // If the hint is inside of a range we don't want
    // to override the classes added by the hint that
    // is a direct child of the item
    if (!inRange) {
      this.bkkrStyle.emit({
        'hint': true,
        [`hint-${position}`]: position !== undefined
      });
    }
  }
